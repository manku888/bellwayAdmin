            ...
            <form action="{{ route('leads-master.store') }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        <label for="bg_color" class="form-label h6">Background Color</label>
                        ...
                        <script>
                            document.getElementById('bg_color').addEventListener('input', function() {
                                ...
                                document.getElementById('colorLabel').style.color = color; // Update label color to match selected color
                            });
                        </script>
                ...
                <table class="table table-striped table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->name }}</td>
                            <td>
                                <span class="badge rounded-pill" style="background-color: {{ $item->bg_color }};">{{ $item->bg_color }}</span>
                            </td>
                            <td>
                                <!-- Active/Inactive Toggle with Form Switch using the provided logic -->
                                <form action="{{ route('leads-master.toggleStatus', $item->id) }}" method="GET">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault{{ $item->id }}" name="status"
                                            {{ $item->status ? 'checked' : '' }}
